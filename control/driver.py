...
class NeutronDbSubnet(ipam_base.Subnet):
    ...
    def _generate_ips(self, context, prefer_next=False, num_addresses=1):
        ...
        LOG.info("-jinlong-_generate_ips--prefer_next: %s num_addresses: %s"
                % (prefer_next, num_addresses))
        allocated_ips = []
        ...
        for ip_pool in self.subnet_manager.list_pools(context):
            ...
            for item in av_ips:
                cur_ip = str(item)
                LOG.info("-jinlong-get order ip: %s" % cur_ip)
                r = self._ip_detection(cur_ip)
                if r == 0:
                    LOG.warning("-jinlong-ip: %s in use, so pass" % cur_ip)
                    continue
                allocated_ip_pool.append(item)
                if i == allocated_num_addresses:
                    break
                i += 1
            LOG.info('-jinlong-random ip_pool: %s' % allocated_ip_pool)
    ...
    def allocate(self, address_request):
        LOG.info('-jinlong-begin allocate ip')
